...
describe('parseCodeowners', () => {
  it('skips comments and blank lines', () => {
    ...
    expect(rules.map(rule => rule.team)).toEqual(['team-docs', 'team-platform'])
  })

  it('ignores owners that do not match the team prefix', () => {
    const rules = parseCodeowners('* @alice @freckle/reviewers @freckle/team-platform', 'team-')
  ...
  it('strips a trailing inline comment before extracting owners', () => {
    const rules = parseCodeowners('docs/ #ask @freckle/team-docs about this', 'team-')
...
describe('resolveTeamForPaths / resolution', () => {
  const content = ['docs/ @freckle/team-docs', 'src/ @freckle/team-platform'].join('\n')

  it('resolves one team when unmatched paths accompany matched ones', () => {
    expect(resolve(content, ['src/main.ts', 'README.md'])).toBe('team-platform')
  })
  ...
  it('falls back when there are no rules at all', () => {
    expect(resolveTeamForPaths([], ['src/main.ts'], 'fallback')).toBe('fallback')
  })
})
