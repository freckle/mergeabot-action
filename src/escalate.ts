...
async function loadCodeowners(inputs: Inputs, client: GitHubClient): Promise<CodeownersRule[]> {
  const content = await client.getFileContent(inputs.codeownersPath)

  if (content === null) {
    core.warning(`Could not read ${inputs.codeownersPath}; using fallback team only`)
    return []
  }

  const rules = parseCodeowners(content, inputs.escalationTeamPrefix)
  core.info(`Loaded ${rules.length} CODEOWNERS rule(s) from ${inputs.codeownersPath}`)
  return rules
}
...
export async function escalateFailingPrs(inputs: Inputs, client: GitHubClient): Promise<void> {
  ...
  const candidates = prs.filter(pr => !pr.hasReviewRequest && pr.hasFailingStatus)
  ...
  for (const pr of candidates) {
    try {
      ...
      const team = resolveTeamForPaths(rules, files, inputs.escalationFallbackTeam)
      ...
      if (inputs.dryRun) {
        core.info(`  => [dry-run] Would request ${team} and post escalation comment`)
        continue
      }

      core.info(`  => Requesting review from ${team} and posting comment`)
      await client.requestReviewers(pr.number, [], [team])
      await client.createComment(pr.number, escalationCommentBody(inputs.escalationCommentSuffix))
    } catch (error: unknown) {
