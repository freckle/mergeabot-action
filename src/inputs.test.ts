...
describe("parseInputs", () => {
  ...
  it("rejects any other strategy", () => {
    expect(() => parseInputs(raw({ strategy: "fast-forward" }))).toThrow(
      /Invalid strategy/,
    );
  });
  ...
  it("rejects a github-repository without a slash", () => {
    expect(() => parseInputs(raw({ repository: "not-a-repo" }))).toThrow(
  ...
  it("treats an empty exclude-title-regex as no filter", () => {
    expect(
      parseInputs(raw({ excludeTitleRegex: "" })).excludeTitleRegex,
    ).toBeNull();
  });
