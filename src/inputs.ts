...
export function parseInputs(raw: RawInputs): Inputs {
  ...
  if (strategy !== 'merge' && strategy !== 'rebase' && strategy !== 'squash') {
    throw new Error(`Invalid strategy: must be merge, rebase, or squash (got "${raw.strategy}")`)
  }

  const [owner, repo] = raw.repository.split('/')
  if (!owner || !repo) {
    throw new Error(`Invalid github-repository: expected "owner/repo", got "${raw.repository}"`)
  }

  return {
    excludeTitleRegex: raw.excludeTitleRegex ? new RegExp(raw.excludeTitleRegex) : null,
